...
describe("IntegrationsClient entrypoint (stubbed fetch)", () => {
  ...
  it("performs a minimal connection flow without real network", async () => {
    ...
    const connection = { id: "conn-1", campgroundId: "camp-123", type: "accounting", provider: "qbo" };
    ...
    fetchMock
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify([connection]), { status: 200, headers: { "content-type": "application/json" } }))
      // trigger sync
      .mockResolvedValueOnce(new Response(JSON.stringify({ status: "queued" }), { status: 200, headers: { "content-type": "application/json" } }))
      // list logs
      .mockResolvedValueOnce(new Response(JSON.stringify(logPage), { status: 200, headers: { "content-type": "application/json" } }))
      // list webhook events
      .mockResolvedValueOnce(new Response(JSON.stringify(webhookPage), { status: 200, headers: { "content-type": "application/json" } }))
      // queue export
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: "export-1" }), { status: 200, headers: { "content-type": "application/json" } }));
    ...
    const exportJob = await client.queueExport({ type: "api", connectionId: created.id, resource: "ledger" });
