...
async function main() {
  ...
  const toRemove = await prisma.$queryRawUnsafe<{id: string, name: string, state: string | null}[]>(`
  ...
  for (const cg of toRemove) {
    ...
    else if (name.includes('visitor center') || name.includes('information')) type = 'Visitor Center';
