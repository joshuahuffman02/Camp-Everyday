...
import { Prisma, ReservationStatus, ReferralIncentiveType, TaxRule, TaxRuleType } from "@prisma/client";
...
export class PublicReservationsService {
    ...
    private async computeTaxes(
        ...
        const inclusiveRateTotal = inclusiveRules.reduce((sum, rule) => sum + Number(rule.rate ?? 0), 0);
        const netBase = inclusiveRateTotal > 0
            ? Math.round(taxableCents / (1 + inclusiveRateTotal))
            : taxableCents;
    ...
    async createReservation(dto: CreatePublicReservationDto) {
        ...
        let quote: Awaited<ReturnType<PublicReservationsService["getQuote"]>> = await this.getQuote(dto.campgroundSlug, {
            siteId: siteId,
                ...
                if (!signed) {
                    ...
                    const signingUrl =
                        signatureResult && typeof signatureResult === "object" && "signingUrl" in signatureResult
                            ? (typeof signatureResult.signingUrl === "string" ? signatureResult.signingUrl : undefined)
                            : undefined;
