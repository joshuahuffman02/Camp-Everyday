...
describe('StoredValueService', () => {
  ...
  describe('ensureTaxableFlag', () => {
    ...
    it('should throw ConflictException when flags mismatch', () => {
      ...
      expect(() => ensureTaxableFlag({ taxableLoad: true }, false)).toThrow('Taxable load flag mismatch');
    });
  ...
  describe('validateTaxableLoad', () => {
    ...
    it('should throw when taxableLoad is true but no campgroundId', async () => {
      ...
      await expect(validateTaxableLoad(null, true)).rejects.toThrow('taxable_load requires campground context');
    });
    ...
    it('should throw when taxableLoad is true but no active tax rule', async () => {
      ...
      await expect(validateTaxableLoad('cg-1', true)).rejects.toThrow('Taxable load requires an active tax rule');
    });
