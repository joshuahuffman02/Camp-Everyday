...
export class PosController {
  ...
  @Patch("carts/:id")
  updateCart(@Param("id") id: string, @Body() dto: UpdateCartDto, @Req() req: any) {
    return this.service.updateCart(id, dto, req.user);
  }

  @Post("carts/:id/checkout")
  checkout(@Param("id") id: string, @Body() dto: CheckoutCartDto, @Req() req: any) {
    return this.service.checkout(id, dto, req.headers["idempotency-key"], req.user);
  }

  @Post("offline/replay")
  replay(@Body() payload: OfflineReplayDto, @Req() req: any) {
