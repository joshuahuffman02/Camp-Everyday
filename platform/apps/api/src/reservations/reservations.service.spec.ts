  ...
  describe('isRigCompatible', () => {
    ...
    describe('RV types', () => {
      ...
      it('should use siteClass max length as fallback', () => {
        expect(isRigCompatible({ siteType: 'rv', rigMaxLength: null, siteClassRigMaxLength: 30 }, 'rv', 35)).toBe(false);
        expect(isRigCompatible({ siteType: 'rv', rigMaxLength: null, siteClassRigMaxLength: 40 }, 'rv', 35)).toBe(true);
      });

      it('should prefer site max length over site class', () => {
        expect(isRigCompatible({ siteType: 'rv', rigMaxLength: 25, siteClassRigMaxLength: 40 }, 'rv', 30)).toBe(false);
      });

      it('should allow any RV length when no max specified', () => {
        expect(isRigCompatible({ siteType: 'rv', rigMaxLength: null, siteClassRigMaxLength: null }, 'rv', 100)).toBe(true);
      });
    });
  ...
  describe('validateAssignmentConstraints', () => {
    ...
    describe('occupancy constraints', () => {
      it('should pass when occupancy within limit', () => {
        expect(() => validateAssignmentConstraints(
