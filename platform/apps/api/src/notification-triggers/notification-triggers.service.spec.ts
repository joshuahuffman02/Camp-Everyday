...
describe('NotificationTriggersService', () => {
  ...
  describe('matchesConditions', () => {
    ...
    describe('simple equality', () => {
      ...
      it('should match multiple conditions', () => {
        ...
        const payload: TriggerPayload = { campgroundId: 'cg-1', guestName: 'John Doe', siteNumber: 'A1' };
        expect(matchesConditions(conditions, payload)).toBe(true);
      });

      it('should fail if any condition does not match', () => {
        const conditions = { guestName: 'John Doe', siteNumber: 'A1' };
        const payload: TriggerPayload = { campgroundId: 'cg-1', guestName: 'John Doe', siteNumber: 'B2' };
        expect(matchesConditions(conditions, payload)).toBe(false);
      });
    });
  ...
  describe('interpolate', () => {
    ...
    it('should replace guest_name placeholder', () => {
      const result = interpolate('Hello {{guest_name}}!', { campgroundId: 'cg-1', guestName: 'John' });
    ...
    it('should replace campground_name placeholder', () => {
      const result = interpolate('Welcome to {{campground_name}}!', { campgroundId: 'cg-1' }, { name: 'Happy Camper RV' });
      expect(result).toBe('Welcome to Happy Camper RV!');
    });

    it('should replace site_number placeholder', () => {
      const result = interpolate('Your site is {{site_number}}', { campgroundId: 'cg-1', siteNumber: 'A15' });
    ...
    it('should replace date placeholders', () => {
      ...
      const result = interpolate(
        'Check-in: {{arrival_date}}, Check-out: {{departure_date}}',
        { campgroundId: 'cg-1', arrivalDate: arrival, departureDate: departure }
      );
    ...
    it('should format amount as currency', () => {
      const result = interpolate('Amount due: {{amount}}', { campgroundId: 'cg-1', amountCents: 15000 });
      expect(result).toBe('Amount due: $150.00');
    });

    it('should replace reservation_id placeholder', () => {
      const result = interpolate('Confirmation #{{reservation_id}}', { campgroundId: 'cg-1', reservationId: 'RES-123' });
    ...
    it('should handle multiple placeholders', () => {
      ...
      expect(result).toBe('Dear John, your reservation RES-456 at Pine Forest Camp is confirmed for B10.');
    });

    it('should handle missing placeholders gracefully', () => {
      const result = interpolate(
        'Site: {{site_number}}, Amount: {{amount}}',
        { campgroundId: 'cg-1' }
      );
