...
import { Prisma, ReservationStatus, SocialContentCategory, SocialPlatform, SocialPostStatus, SocialSuggestionStatus, SocialSuggestionType } from '@prisma/client';
...
export type SocialPlannerStore = {
    socialPost: Pick<PrismaServiceType["socialPost"], "findMany" | "findUnique" | "create" | "update" | "delete" | "count">;
    socialTemplate: Pick<PrismaServiceType["socialTemplate"], "findMany" | "create" | "update" | "delete" | "count">;
    socialContentAsset: Pick<PrismaServiceType["socialContentAsset"], "findMany" | "create" | "update" | "delete">;
    ...
    socialOpportunityAlert: Pick<PrismaServiceType["socialOpportunityAlert"], "create" | "findMany" | "update">;
...
export class SocialPlannerService {
    ...
    async getPost(id: string) {
        return this.prisma.socialPost.findUnique({ where: { id }, include: { SocialTemplate: true, SocialSuggestion: true } });
    }
    ...
    async updatePost(id: string, dto: UpdatePostDto) {
        const data: Prisma.SocialPostUpdateInput = {
            ...
            SocialTemplate: dto.templateId ? { connect: { id: dto.templateId } } : dto.templateId === null ? { disconnect: true } : undefined,
            SocialSuggestion: dto.suggestionId ? { connect: { id: dto.suggestionId } } : dto.suggestionId === null ? { disconnect: true } : undefined,
            updatedAt: new Date(),
        };
    ...
    async updateAsset(id: string, dto: UpdateAssetDto) {
        const data: Prisma.SocialContentAssetUpdateInput = {
            ...
            tags: dto.tags,
            notes: dto.notes,
            User: dto.uploadedById ? { connect: { id: dto.uploadedById } } : dto.uploadedById === null ? { disconnect: true } : undefined,
            updatedAt: new Date(),
        };
        ...
        events.forEach((event) => {
            ...
            if (utilization < 0.5) {
                suggestions.push({
                    ...
                    reason: toJsonValue({ utilization, capacity: event.capacity, startDate: event.startDate }),
                    createdAt: now,
                    updatedAt: now,
                });
            } else {
