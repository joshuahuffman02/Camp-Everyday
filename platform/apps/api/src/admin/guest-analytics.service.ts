...
export class GuestAnalyticsService {
    ...
    async getOverview(range: string): Promise<GuestAnalyticsOverview> {
        ...
        const avgStayLength = reservationsWithDates.length > 0
            ? totalNights / reservationsWithDates.length
            : 0;
    ...
    async getSeasonalTrends(range: string): Promise<SeasonalTrends> {
        ...
        const monthlyAggregates = new Map<string, { reservations: number; revenue: number; stayLength: number; count: number }>();
        ...
        const byMonth = monthNames.map((month) => {
            ...
            return {
                ...
                avgStayLength: data && data.count > 0
                    ? Math.round((data.stayLength / data.count) * 10) / 10
                    : 0,
            };
        });
    ...
    async generateInsights(range: string): Promise<Insight[]> {
        ...
        if (insights.length === 0) {
            insights.push({
                ...
                description: "Guest analytics are being collected. Insights will appear as more data becomes available.",
                type: "info",
            });
        }
    ...
    async getFullAnalytics(range: string) {
        const [overview, geographic, demographics, seasonalTrends, travelBehavior, insights] = await Promise.all([
