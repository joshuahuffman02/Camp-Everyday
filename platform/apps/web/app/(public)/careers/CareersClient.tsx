  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16 space-y-10">
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
        ...
        <p className="text-muted-foreground text-sm">We're pre-launch and gathering interest. Tell us about you—roles will open as we scale.</p>
        <div className="space-y-2">
          {roles.map((role) => (
            <div key={role.title} className="flex items-center justify-between bg-muted border border-border rounded-lg px-4 py-3">
              <div>
                <div className="font-semibold text-foreground">{role.title}</div>
                <div className="text-xs text-muted-foreground">{role.location} • {role.type}</div>
              </div>
              <span className="text-xs font-semibold text-keepr-evergreen bg-keepr-evergreen/10 px-2 py-1 rounded-full">Pipeline</span>
            </div>
          ))}
        </div>
        ...
        <p className="text-xs text-muted-foreground">We welcome generalists and outdoors lovers—even if you don't see the perfect title here.</p>
      </div>
      ...
      <div className="bg-keepr-charcoal text-white rounded-2xl p-6 flex flex-col md:flex-row items-center gap-4 justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm text-white/70">Tools: Next.js, React, TypeScript, Postgres, Prisma, Tailwind</div>
        </div>
      </div>
    </div>
  );
