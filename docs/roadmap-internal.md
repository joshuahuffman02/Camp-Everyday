...
- Scope: gift cards/store credit (issue/reload/redeem/void/refund-to-credit, PIN optional, idempotent ledger, taxable-load flag, liability snapshots); POS (split tender incl. gift card/charge-to-site, receipts, returns/exchanges, offline replay with seq dedupe); waitlist with auto-offers (holds, throttle, idempotent accept/decline/expire); reporting (ADR/RevPAR/revenue/channel mix/gift card liability, saved filters, CSV export, scheduled email); observability/rate limits/redaction.
- Data/API: SQL DDL drafted (gift_card + ledger, POS orders/lines/tenders/returns + terminal sessions, waitlist entries/holds/offers, facts/dims + liability_snapshot); OpenAPI 3.1 draft for gift cards, POS, waitlist, reports; shared idempotency + tenant scoping/RLS; comms triggers/templates enumerated (gift card events, receipts/refunds, offers/reminders/outcomes, report ready).
- Backlog: import-ready backlog CSV with epics/tasks/owners/points/due dates at `docs/phase4-backlog.csv` (gift cards, POS, waitlist, reporting, foundations/observability).
- Gating decisions (due Dec 17): tokenization/processor vs internal (PCI scope, Alex); tax engine + taxable-load rules (Priya); email/SMS provider + template owner (Sam). Due Dec 20: POS hardware/offline limits (Taylor); POS/gift card/waitlist UX mocks (Jordan). Infra: idempotency store + observability wiring + staging creds (Casey, Dec 24).
- Go/No-Go (target Jan 3): see `docs/phase4-go-no-go.md` — idempotency enforced; RLS/audit on new tables; rate limits validated; double-redeem race blocked; offline replay dedupe with backlog alert; waitlist throttle/expiry/accept idempotent; reports capacity guard 503 under load; comms templates QA’d; liability snapshot = sum of balances; PII/PAN redaction verified; alerts firing in staging (redeem failure, offline backlog, offer lag, report failures); synthetic checks for redeem/POS/offer/export passing; alert env/targets set (`PAGERDUTY_SERVICE_KEY`, `SLACK_ALERT_WEBHOOK`, `METRICS_NAMESPACE=campreserv`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `ENABLE_READY_PROBE`, `ENABLE_COMMS_METRICS`, `ENABLE_OTA_MONITORING`) with validation steps for queue lag, OTA backlog, comms bounce/complaint, API/Web perf budgets, and staging validation checkboxes for alerts + synthetics.
